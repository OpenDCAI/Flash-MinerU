...
from ..utils.config_reader import get_table_enable, get_llm_aided_config
...
if llm_aided_config:
    ...
    if title_aided_config.get('enable', False):
        try:
            ...
            from .model_init import atom_ocr_model_init
...
def blocks_to_page_info(page_blocks, image_dict, image_writer, page_index) -> dict:
    """将blocks转换为页面信息"""
    ...
    width, height = image_dict["page_width"], image_dict["page_height"]
...
def result_to_middle_json(model_output_blocks_list, images_list, image_writer):
    ...
    for index, page_blocks in enumerate(model_output_blocks_list):
        ...
        page_info = blocks_to_page_info(page_blocks, image_dict, image_writer, index)
